...
\textbf{针对问题一}，给定FY1无人机以120 m/s朝向假目标飞行，在1.5 s后投放1枚弹、3.6 s后起爆的条件下，通过精确计算导弹轨迹、云团轨迹和遮蔽几何关系，得出有效遮蔽时长为\textbf{1.392 s}，遮蔽时间区间为$[8.06, 9.45]$ s。

\textbf{针对问题二}，以无人机飞行方向、飞行速度、投放时刻和起爆延迟为决策变量，建立了以有效遮蔽时长为目标的非线性优化模型。采用粒子群优化算法（PSO）求解，得到最优策略下遮蔽时长为\textbf{4.577 s}，相比问题一提升了\textbf{228.8\%}。最优策略为：飞行方向角$8.20^\circ$，飞行速度$91.39$ m/s，投放时刻$0.000$ s，起爆延迟$0.900$ s。

\textbf{针对问题三}，FY1投放3枚弹，考虑投放间隔约束（$\geq 1$ s），建立了8维优化模型。通过PSO优化，实现了3枚弹的时间接力遮蔽，总有效遮蔽时长达\textbf{4.575 s}。

\textbf{针对问题四}，FY1、FY2、FY3各投放1枚弹，建立了12维协同优化模型。通过分阶段优化策略，充分利用3架无人机的空间分布优势，总遮蔽时长达\textbf{3.679 s}。

\textbf{针对问题五}，5架无人机各至多投放3枚弹干扰3枚导弹，建立了大规模优化模型。采用分层优化策略：首先基于几何关系进行任务分配，然后对每枚导弹独立优化投放策略。最终实现对M1、M2、M3的总遮蔽时长分别为\textbf{8.650 s}、\textbf{0.000 s}、\textbf{4.700 s}，总计\textbf{13.350 s}。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/q1_3d_trajectory.png}
    \caption{烟幕遮蔽几何示意图与问题1轨迹}
...
计算结果：使用点目标近似时有效遮蔽时长为\textbf{1.435 s}，使用圆柱体目标精确模型时有效遮蔽时长为\textbf{1.392 s}，遮蔽时间区间为$[8.06, 9.45]$ s。两者差异较小（约3.0\%），说明点目标近似在本问题中具有良好的精度。

\subsubsection{问题1结果汇总}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/q1_3d_trajectory.png}
        \caption{3D轨迹图}
        \label{fig:q1_3d}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/q1_distance_time.png}
        \caption{遮蔽距离-时间曲线}
        \label{fig:q1_dist}
    \end{subfigure}
    \caption{问题1计算结果}
...
由于本问题的可行域较小（遮蔽条件对参数敏感），纯随机初始化的PSO难以找到可行解。因此采用\textbf{引导初始化策略}：将部分粒子初始化在已知较好解附近（如问题1的参数），在种子点周围添加5\%范围的随机扰动，确保PSO从有希望的区域开始搜索。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/q2_convergence.png}
    \caption{问题2 PSO收敛曲线}
...
\begin{table}[H]
    ...
    \begin{tabular}{ll}
        ...
        参数 & 最优值 \\
        \midrule
        飞行方向角$\theta$ & $8.20^\circ$ \\
        飞行速度$v_u$ & 91.39 m/s \\
        投放时刻$t_d$ & 0.000 s \\
        起爆延迟$\Delta t$ & 0.900 s \\
        \midrule
        有效遮蔽时长 & 4.577 s \\
        \bottomrule
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{cccccc}
        ...
        \midrule
        1 & 1.500 & 3.600 & (17620,0,1800) & (17188,0,1736) & 1.392 \\
        2 & 3.500 & 3.600 & (17380,0,1800) & (16948,0,1736) & 1.580 \\
        3 & 5.500 & 3.600 & (17140,0,1800) & (16708,0,1736) & 1.603 \\
        \midrule
        \multicolumn{5}{r}{总有效遮蔽时长} & 4.575 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/q3_convergence.png}
        \caption{PSO收敛曲线}
        \label{fig:q3_3d}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/q3_timeline.png}
        \caption{遮蔽时间甘特图}
...
\begin{table}[H]
    ...
    \begin{tabular}{cccccc}
        ...
        \midrule
        FY1 & 163.51 & 78.67 & 27.866 & 4.160 & --- \\
        FY2 & 219.63 & 139.46 & 6.586 & 8.487 & --- \\
        FY3 & 189.75 & 103.08 & 17.274 & 2.598 & --- \\
        \midrule
        \multolumn{5}{r}{总有效遮蔽时长} & 3.679 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/q4_3d_trajectory.png}
    \caption{问题4：3架无人机协同3D轨迹图}
...
\begin{table}[H]
    ...
    \begin{tabular}{cccc}
        ...
        导弹 & 分配的无人机 & 遮蔽时长(s) & 备注 \\
        \midrule
        M1 & FY1, FY2 & 8.650 & FY1、FY2协同遮蔽 \\
        M2 & FY4 & 0.000 & FY4未能找到有效遮蔽窗口 \\
        M3 & FY3, FY5 & 4.700 & FY3、FY5协同遮蔽 \\
        \midrule
        \multicolumn{3}{r}{\textbf{总遮蔽时长}} & \textbf{13.350} \\
        \bottomrule
    \end{tabular}
\end{table}

问题5的求解中，对M2的遮蔽时长为0，这是因为FY4的初始位置$(11000, 2000, 1800)$相对于M1的初始位置$(20000, 0, 2000)$而言，其可投放的烟幕弹难以在M1的视线路径上形成有效遮蔽。在分层优化中，FY4被分配给M2，但由于M2的几何位置和FY4的初始条件限制，PSO未能在有限迭代次数内找到可行解。这提示我们在实际应用中需要更精细的任务分配策略或更长的优化时间。
...
五个问题的求解结果如表~\ref{tab:summary} 所示。
...
\begin{table}[H]
    ...
    \begin{tabular}{ccccc}
        \toprule
        问题 & 决策变量数 & 优化算法 & 有效遮蔽时长(s) & 相比问题1提升 \\
        \midrule
        问题1 & 0（给定） & 直接计算 & 1.392 & --- \\
        问题2 & 4 & PSO & 4.577 & 228.8\% \\
        问题3 & 8 & PSO & 4.575 & 228.6\% \\
        问题4 & 12 & PSO & 3.679 & 164.3\% \\
        问题5 & 40 & 分层PSO & 13.350 & --- \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{问题2 vs 问题1}：通过优化飞行方向和投放参数，遮蔽时长提升了约3倍。最优策略的关键在于选择合适的起爆延迟（0.9 s），使云团在导弹视线路径上形成有效遮蔽。

    \item \textbf{问题3 vs 问题2}：3枚弹的遮蔽时长（4.575 s）与1枚弹（4.577 s）基本持平。这表明在当前约束条件下，单枚弹已经能够覆盖主要的遮蔽窗口，额外的弹主要用于填补空隙。

    \item \textbf{问题4}：3架无人机的遮蔽时长（3.679 s）略低于单架无人机（4.577 s）。这是因为FY2和FY3距离M1较远，其烟幕弹到达遮蔽位置的时间较长，限制了有效遮蔽窗口。

    \item \textbf{问题5}：5架无人机对3枚导弹的总遮蔽时长为13.350 s。其中对M1的遮蔽效果最好（8.650 s），对M2的遮蔽时长为0（FY4的位置限制），对M3的遮蔽时长为4.700 s。
\end{enumerate}
...
灵敏度分析的具体数据如表~\ref{tab:sensitivity_data} 所示，各参数对遮蔽时长的影响趋势清晰可见。

\subsection{灵敏度分析数据}

以问题2最优策略（$\theta = 8.20^\circ$，$v_u = 91.39$ m/s，$t_d = 0$ s，$\Delta t = 0.9$ s）为基准，分别考察各参数变化对遮蔽时长的影响。
...
\begin{table}[H]
    ...
    \begin{tabular}{cccccc}
        \toprule
        $R$(m) & $T_{\text{shield}}$(s) & $T_{\text{eff}}$(s) & $T_{\text{shield}}$(s) & $v_c$(m/s) & $T_{\text{shield}}$(s) \\
        \midrule
        5  & 2.410 & 10 & 4.820 & 1.0 & 5.520 \\
        6  & 2.890 & 12 & 4.820 & 1.5 & 5.600 \\
        7  & 3.370 & 14 & 4.820 & 2.0 & 5.460 \\
        8  & 3.850 & 16 & 4.820 & 2.5 & 5.170 \\
        9  & 4.330 & 18 & 4.820 & 3.0 & 4.820 \\
        10 & 4.820 & 20 & 4.820 & 3.5 & 4.430 \\
        11 & 5.100 & 22 & 4.820 & 4.0 & 4.070 \\
        12 & 5.330 & 24 & 4.820 & 4.5 & 3.730 \\
        13 & 5.560 & 26 & 4.820 & 5.0 & 3.420 \\
        14 & 5.790 & 28 & 4.820 &  &  \\
        15 & 6.010 & 30 & 4.820 &  &  \\
        \bottomrule
    \end{tabular}
...
\begin{enumerate}
    \item \textbf{云团有效半径$R$}：遮蔽时长对$R$最敏感，呈近似线性关系。$R$从5 m增加到15 m时，遮蔽时长从2.41 s增加到6.01 s，增幅约150\%。$R$每增加1 m，遮蔽时长约增加0.36 s。

    \item \textbf{有效遮蔽时间$T_{\text{eff}}$}：在$T_{\text{eff}} \in [10, 30]$ s范围内，遮蔽时长恒为4.82 s，不受$T_{\text{eff}}$影响。这是因为当前最优策略的遮蔽窗口（约4.82 s）完全包含在$T_{\text{eff}}$范围内，$T_{\text{eff}}$不是约束瓶颈。

    \item \textbf{云团下沉速度$v_c$}：$v_c$对遮蔽时长有显著负向影响。$v_c$从1.0 m/s增加到5.0 m/s时，遮蔽时长从5.52 s下降到3.42 s，降幅约38\%。这是因为$v_c$增大会使云团更快偏离导弹视线。

    \item \textbf{导弹速度$v_m$}：$v_m$对遮蔽时长有中等负向影响。$v_m$从250 m/s增加到350 m/s时，遮蔽时长从5.23 s下降到4.36 s，降幅约17\%。$v_m$增大使导弹更快飞过遮蔽区域。
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{PSO可能陷入局部最优}：高维问题中，单次PSO运行可能无法找到全局最优。问题5中对M2的遮蔽时长为0，可能是PSO未能在有限迭代内找到可行解。

    \item \textbf{任务分配策略较简单}：问题5中采用基于几何关系的固定任务分配，未考虑动态调整。FY4被固定分配给M2，但其初始位置可能不利于遮蔽M2。
\end{enumerate}
